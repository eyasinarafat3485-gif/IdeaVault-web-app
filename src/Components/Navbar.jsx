        ...
        <nav className='sticky z-50 w-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-gray-100 dark:border-slate-800 transition-colors duration-300'>
            <div className='max-w-7xl mx-auto flex justify-between items-center h-20 px-5 lg:px-14'>
                ...
                <div className='flex items-center gap-3 md:gap-4'>
                    <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className="p-2.5 rounded-xl cursor-pointer bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-yellow-400 hover:ring-2 ring-indigo-400 transition-all shadow-sm flex items-center justify-center" aria-label="Toggle Theme">
                    ...
                    {user ? (
                        <div className='flex items-center gap-3'>
                            ...
                            <Button onClick={handleSignOut} size="sm" className='hidden md:flex rounded-sm bg-red-500 font-bold text-white' >
                                Logout
                            </Button>
                        </div>
                    ) : (
                        <div className='hidden lg:flex items-center gap-4'>
                            <Link href="/login" passHref>
                                <Button variant="outline" className="font-semibold rounded-md dark:text-white text-indigo-600 dark:text-indigo-400">Login</Button>
                            </Link>
                            <Link href="/register" passHref>
                                <Button color="primary" className="font-semibold rounded-md bg-indigo-600 text-white shadow-md">Register</Button>
                            </Link>
                        </div>
                    )}
                        ...
                        >
                            <div className='flex flex-col gap-2'>
                                <ul className='flex flex-col gap-1'>
                                    {navLinks.map((link) => (
                                        <li key={link.href}>
                                            <Link onClick={() => setIsOpen(false)} href={link.href} className={`text-lg font-medium block p-2 rounded-md transition-all ${pathname === link.href ? 'text-indigo-600 bg-indigo-50 dark:bg-indigo-500/10' : 'text-gray-800 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-slate-800'}`} >
                                                {link.name}
                                            </Link>
                                        </li>
                                    ))}
                                ...
                                <div className='flex flex-col gap-2'>
                                    {user ? (
                                        ...
                                        <>
                                            <Link href="/login" passHref>
                                                <Button onClick={() => setIsOpen(false)} variant="flat" className="w-full h-11 font-bold text-indigo-600 dark:text-indigo-400">
                                                    Login
                                                </Button>
                                            </Link>
                                            <Link href="/register" passHref>
                                                <Button onClick={() => setIsOpen(false)} className="w-full font-bold h-11 bg-indigo-600 text-white">
                                                    Register
                                                </Button>
                                            </Link>
                                        </>
